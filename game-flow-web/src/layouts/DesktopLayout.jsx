...
const links = [['Home', '/app/home'], ['Explore', '/app/explore'], ['Upload', '/app/upload'], ['Inbox', '/app/inbox'], ['Profile', '/app/profile']]
...
export default function DesktopLayout() {
  ...
  return <div className="desktop-shell">
    <aside className="side-nav"><NavLink className="brand" to="/app/home">game<span>flow</span></NavLink><nav>{links.map(([label, to]) => <NavLink key={to} to={to}>{label}</NavLink>)}</nav><div className="nav-foot"><div className="user-chip"><span>{(user?.name || user?.username || 'G').slice(0, 1)}</span><div><b>{user?.name || user?.username}</b><small>{connectionState}</small></div></div><button className="text-button" onClick={() => { logout(); navigate('/signin') }}>Sign out</button><a className="text-button" href="/m">Mobile view</a></div></aside>
    <section className="main-area"><header className="topbar"><div><strong>GameFlow</strong><small>Creative work, in motion</small></div><NavLink className="primary-button" to="/app/upload">New project</NavLink></header><Outlet /></section>
