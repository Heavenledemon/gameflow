const STORAGE_KEY = 'gameflow:publish-draft:v1'
...
export function loadPublishDraft() {
  try {
    ...
    return {
      ...
      software: Array.isArray(parsed.software) ? parsed.software.filter((value) => typeof value === 'string').slice(0, 30) : DEFAULT_PUBLISH_DRAFT.software,
...
export function savePublishDraft(draft, fileState = {}) {
  ...
  try {
    ...
    // Draft recovery is best-effort; publishing must remain available when storage is blocked.
  }
}
...
export function clearPublishDraft() {
  try {
    ...
    // Nothing else should fail when browser storage is unavailable.
  }
}

export function hasMeaningfulPublishDraft(draft, fileState = {}) {
  return Boolean(
    draft.title.trim() || draft.description.trim() || draft.tags.length ||
    draft.type !== DEFAULT_PUBLISH_DRAFT.type || draft.mode !== DEFAULT_PUBLISH_DRAFT.mode ||
    draft.visibility !== DEFAULT_PUBLISH_DRAFT.visibility || fileState.hadFiles || fileState.hadCover
  )
}
