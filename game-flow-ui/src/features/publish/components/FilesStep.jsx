...
export default function FilesStep({ assets, cover, gameplayGif, showCover = true, statuses = {}, onRemoveAsset, onRemoveCover, onRemoveGameplayGif, retryAvailable, onRetry, retrying = false }) {
  const totalSize = assets.reduce((sum, item) => sum + item.file.size, 0) + (cover?.file.size || 0) + (gameplayGif?.file.size || 0)
  return <section className="publish-files" aria-labelledby="publish-files-title">
    ...
    {showCover ? <div className="publish-cover-row"><div><strong>Cover thumbnail</strong><span>{cover ? `${cover.file.name} · ${formatBytes(cover.file.size)}` : 'No cover selected'}</span></div>{cover && onRemoveCover ? <button type="button" onClick={onRemoveCover}>Remove</button> : null}{cover ? <span className={`publish-file-row__status publish-file-row__status--${statuses[cover.relativePath]?.status || 'ready'}`}>{statuses[cover.relativePath]?.status === 'uploading' ? 'Uploading…' : statuses[cover.relativePath]?.status === 'complete' ? 'Uploaded' : statuses[cover.relativePath]?.status === 'failed' ? 'Upload failed' : 'Ready'}</span> : null}</div> : null}
    {gameplayGif ? <div className="publish-cover-row"><div><strong>Gameplay GIF</strong><span>{gameplayGif.file.name} · {formatBytes(gameplayGif.file.size)}</span></div><button type="button" onClick={onRemoveGameplayGif}>Remove</button><span className={`publish-file-row__status publish-file-row__status--${statuses[gameplayGif.relativePath]?.status || 'ready'}`}>{statuses[gameplayGif.relativePath]?.status === 'uploading' ? 'Uploading…' : statuses[gameplayGif.relativePath]?.status === 'complete' ? 'Uploaded' : statuses[gameplayGif.relativePath]?.status === 'failed' ? 'Upload failed' : 'Ready'}</span></div> : null}
  </section>
}
