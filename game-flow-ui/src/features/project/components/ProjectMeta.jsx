...
function MetadataRow({ label, children }) {
  ...
  return <div className="project-meta__row"><dt>{label}</dt><dd>{children}</dd></div>
}
...
export default function ProjectMeta({ model, project, expanded, onToggle }) {
  ...
  const collaboratorSummary = collaborators.map((member) => {
    const name = member.name || member.username
    return name ? `${name}${member.role ? ` (${member.role})` : ''}` : null
  }).filter(Boolean).join(', ')
  const hasDetails = Boolean(model.summary || model.tools.length || model.tags.length || platform || project.status || roles.length || collaboratorSummary)
    ...
    <section className="project-meta" aria-labelledby="project-meta-title">
      <div className="project-meta__heading">
        <div>
          ...
          <p>{model.category || model.projectType || 'Project'} <span aria-hidden="true">·</span> {createdLabel}</p>
        </div>
        {hasDetails ? <button type="button" aria-expanded={expanded} aria-controls="project-metadata-details" onClick={onToggle}>{expanded ? 'Hide details' : 'Project details'}</button> : null}
      </div>
      <div id="project-metadata-details" hidden={!expanded} className="project-meta__details">
        {model.summary ? <p className="project-meta__summary">{model.summary}</p> : <p className="project-meta__summary">No description provided for this project.</p>}
        <dl>
        ...
        {model.tools.length ? <div className="project-meta__chips" aria-label="Tools used">{model.tools.map((tool) => <Chip key={tool}>{tool}</Chip>)}</div> : null}
        {model.tags.length ? <div className="project-meta__chips" aria-label="Project tags">{model.tags.map((tag) => <Chip key={tag}>#{tag}</Chip>)}</div> : null}
      </div>
    </section>
  )
}
