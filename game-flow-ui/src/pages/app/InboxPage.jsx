  ...
  return <div className="inbox-list">
    {items.map((item) => {
      ...
      return <article className="inbox-card" key={item.id}>
        <button className="inbox-card__body" type="button" onClick={() => item.conversationId && onOpen(item.conversationId)} disabled={!item.conversationId} aria-label={item.conversationId ? `Open conversation for ${item.project?.title || 'project'}` : undefined}>
...
function ConversationList({ kind, items, status, error, nextCursor, onLoadMore, onRetry, onOpen }) {
  ...
  return <div className="inbox-list">
    {conversations.map((conversation) => <button className="inbox-card inbox-card--conversation" type="button" key={conversation.id} onClick={() => onOpen(conversation.id)}>
