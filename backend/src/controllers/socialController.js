...
export const getPublicUser = asyncHandler(async (request, response) => {
  ...
  const user = await User.findOne(query).select('username name avatar banner headline creatorType isVerified bio description location website skills github itchio behance artstation instagram linkedin companionType companionMotion companionEmotion companionBubble companionBubbleText companionBubbleBehavior companionFootprintsEnabled profileDisplayType profileDesignType profileDesignPalette profileDesignDensity profileDesignLineStyle profileDesignMotion profileDesignInteraction profileDesignDoodleTheme').lean()
